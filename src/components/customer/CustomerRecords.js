...
const configs = {
  bids: { title: "My bids", eyebrow: "Bidding history", url: APIConstants.customerBids, columns: ["Cycle", "Group", "Discount", "Status"], cells: (row) => [row.cycleNumber, row.groupName || `Group #${row.groupId}`, `₹${Number(row.discountAmount).toLocaleString("en-IN")}`, row.status] },
  claims: { title: "My claims", eyebrow: "Payout claims", url: APIConstants.customerClaims, columns: ["Cycle", "Group", "Note", "Status"], cells: (row) => [row.cycleNumber, row.groupName || `Group #${row.groupId}`, row.note || "-", row.status] },
  payments: { title: "My payments", eyebrow: "Contributions and payouts", url: APIConstants.customerPayments, columns: ["Type", "Cycle", "Due / paid", "Group", "Amount", "Method", "Status"], cells: (row) => [row.recordType === "PAYOUT" ? "Payout" : "Contribution", row.cycleNumber || `#${row.cycleId}`, row.paidAt || row.dueDate || "-", row.groupId, `₹${Number(row.amount).toLocaleString("en-IN")}`, row.method || "Manual", row.status] }
};
...
export default function CustomerRecords({ type }) {
  ...
  const rows = data || [];
  return <Shell><Eyebrow>{config.eyebrow}</Eyebrow><Title>{config.title}</Title><Section><Meta>{isLoading ? "Loading records..." : error ? "Unable to load records" : `${rows.length} records`}</Meta><TableWrap><Table><thead><tr>{config.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={row.id}>{config.cells(row).map((cell, index) => <td key={`${row.id}-${index}`}>{index === config.columns.length - 1 ? <Status $warning={cell === "PENDING"}>{cell}</Status> : cell}</td>)}</tr>) : <tr><td colSpan={config.columns.length}>{isLoading ? "Loading records..." : error ? "Unable to load records." : `No ${type} found.`}</td></tr>}</tbody></Table></TableWrap></Section></Shell>;
}
